...
class _HomeFeedScreenState extends State<HomeFeedScreen> {
  ...
  void _toggleDashboard() { HapticFeedback.selectionClick(); setState(() => _isDashboardOpen = !_isDashboardOpen); }
  void _closeDashboard() { if (_isDashboardOpen) setState(() => _isDashboardOpen = false); }

  @override
  Widget build(BuildContext context) {
    var state = context.watch<AppState>();
    return GestureDetector(onTap: _closeDashboard, child: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const PrayaLogo(size: 24), GestureDetector(onTap: () => _showNotifications(context, state), child: Stack(alignment: Alignment.topRight, children: [const Padding(padding: EdgeInsets.all(8.0), child: Icon(Icons.notifications_none, color: Colors.white38, size: 28)), if (state.notifications.isNotEmpty) Positioned(right: 8, top: 8, child: Container(width: 8, height: 8, decoration: const BoxDecoration(color: Colors.redAccent, shape: BoxShape.circle)))]))]), 
      const SizedBox(height: 10), 
      
      // DASHBOARD TOGGLE
      Center(child: GestureDetector(onTap: _toggleDashboard, child: AnimatedContainer(duration: 300.ms, padding: const EdgeInsets.all(5), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(20)), child: Icon(_isDashboardOpen ? Icons.expand_less : Icons.expand_more, color: Colors.white38, size: 20)))),

      AnimatedCrossFade(
        firstChild: const SizedBox(width: double.infinity),
        secondChild: Column(children: [const SizedBox(height: 20), GlassPanel(glow: true, onTap: () => state.setIndex(1), child: Column(children: [Row(children: [Icon(Icons.park, color: state.moodColor), const SizedBox(width: 10), Text(state.getLevelData(state.level).title.toUpperCase(), style: GoogleFonts.cinzel(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.white)), const Spacer(), Text("Level ${state.level}", style: TextStyle(color: state.moodColor))]), const SizedBox(height: 15), LinearProgressIndicator(value: state.levelProgress, backgroundColor: Colors.white10, color: state.moodColor, minHeight: 8, borderRadius: BorderRadius.circular(5)), const SizedBox(height: 8), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("${state.auraPoints} XP", style: const TextStyle(fontSize: 10, color: Colors.white54)), Text("Do dalšího: ${state.xpMissing} XP", style: TextStyle(fontSize: 10, color: state.moodColor))])])), const SizedBox(height: 15), GlassPanel(child: Column(children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("Biofeedback", style: TextStyle(fontSize: 12, color: Colors.white54)), Icon(Icons.circle, size: 8, color: state.moodColor).animate(onPlay: (c)=>c.repeat(reverse: true)).scale()]), const SizedBox(height: 10), SliderTheme(data: SliderThemeData(trackHeight: 6, thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 10), activeTrackColor: state.moodColor, thumbColor: Colors.white), child: Slider(value: state.currentStress, onChanged: (v) => state.updateStress(v)))]),), const SizedBox(height: 20), const Divider(color: Colors.white10)]),
        crossFadeState: _isDashboardOpen ? CrossFadeState.showSecond : CrossFadeState.showFirst, duration: 400.ms, sizeCurve: Curves.easeInOutQuart
      ),

      const SizedBox(height: 20),

      // DENNÍ CITÁT (S FUNKČNÍM ULOŽENÍM)
...
class _DailyQuoteCard extends StatelessWidget {
  ...
  void _askAura(BuildContext context) {
    ...
    // Otevře Auru přes externí modal
      ...
      child: Column(
        ...
        children: [
          ...
          Row(mainAxisAlignment: MainAxisAlignment.end, children: [
              // TLAČÍTKO ULOŽIT (NYNÍ FUNKČNÍ)
              IconButton(
                onPressed: () {
                   context.read<AppState>().saveQuoteAsJournalEntry(quote); // Volání nové metody
