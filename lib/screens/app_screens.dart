  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        GlassPanel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Icon(Icons.circle, size: 8, color: state.moodColor), const SizedBox(width: 5), const Text("Aktivita", style: TextStyle(fontSize: 10, color: Colors.white54)), const SizedBox(width: 15),
...
class TrendChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(pathFill, paintFill); canvas.drawPath(pathLine, paintLine);
  }
...
class CharityScreen extends StatelessWidget { const CharityScreen({super.key}); @override Widget build(BuildContext context) { var state = context.watch<AppState>(); return SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(children: [const SizedBox(height: 20), Text("Dopad", style: GoogleFonts.cinzel(fontSize: 28)), const SizedBox(height: 30), 
        ...
        const SizedBox(height: 30), ...state.charityProjects.map((p) => Padding(padding: const EdgeInsets.only(bottom: 15), child: GlassPanel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(p.title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)), InkWell(onTap: () => state.allocateCharity(p.title), child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: p.color.withOpacity(0.2), borderRadius: BorderRadius.circular(20), border: Border.all(color: p.color)), child: Text("PODPOŘIT", style: TextStyle(color: p.color, fontSize: 10, fontWeight: FontWeight.bold))))]),
