...
// Generativní umění na základě textu (Seed)
...
class RadarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 5; i++) {
      double angle = random.nextDouble() * 2 * pi + (progress * pi);
      double dist = 30 + random.nextDouble() * 50;
      canvas.drawCircle(center + Offset(cos(angle) * dist, sin(angle) * dist),
          3, dotPaint);
    }
  }
