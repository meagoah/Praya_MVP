...
class FeedItem {
  ...
  String author;
  String country;
  String originalText; // Originální jazyk
  String translatedText; // Český překlad
  bool showTranslation; // Přepínač
  int likes;
  bool isLiked;
...
class AppState extends ChangeNotifier {
  ...
  void createPost(String text, double stress) {
    // Přidáme nový post (jakože od tebe)
    feed.insert(0, FeedItem(
      id: DateTime.now().toString(),
      author: nickname.isEmpty ? "Ty" : nickname,
      country: "CZ",
      originalText: text,
      translatedText: text, // U tebe je to stejné
      showTranslation: true // U tebe rovnou česky
    ));
    auraPoints += 50;
...
// OPRAVENÁ KOMPONENTA S PARAMETREM OPACITY
class GlassPanel extends StatelessWidget {
  final Widget child;
  final VoidCallback? onTap;
  final bool glow;
  final double opacity; // Přidáno zpět
  ...
  const GlassPanel({
    ...
    this.onTap, 
    this.glow = false,
    this.opacity = 0.03 // Defaultní hodnota
  });
        ...
        child: BackdropFilter(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: Colors.white.withValues(alpha: opacity), // Použití opacity
              borderRadius: BorderRadius.circular(24), 
...
class MainLayout extends StatelessWidget {
        ...
        children: [
          ...
          SafeArea(
            child: IndexedStack(
              ...
              children: const [HomeFeedScreen(), JourneyScreen(), CreateScreen(), InsightsScreen()],
            ),
          ),
          // 3. Vrstva: DOCK + CREATE BUTTON
          Align(alignment: Alignment.bottomCenter, child: _buildAdvancedDock(context, state)),
          
          // 4. Vrstva: AURA (Always visible)
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          // CREATE BUTTON (Central & Highlighted)
          GestureDetector(
            onTap: () { HapticFeedback.mediumImpact(); state.setIndex(2); },
            child: Container(width: 55, height: 55, decoration: BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [state.moodColor, Colors.purple]), boxShadow: [BoxShadow(color: state.moodColor.withValues(alpha: 0.4), blurRadius: 15)]), child: const Icon(Icons.add, color: Colors.white, size: 30)),
  ...
  Widget _dockItem(IconData icon, int index, AppState state) {
    ...
    return GestureDetector(onTap: () { HapticFeedback.lightImpact(); state.setIndex(index); }, child: AnimatedContainer(duration: 300.ms, padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: active ? Colors.white10 : Colors.transparent, shape: BoxShape.circle), child: Icon(icon, color: active ? state.moodColor : Colors.white38, size: 24)));
  }
}

// --- 6. SCREENS ---

// A. HOME FEED (S PŘEKLADEM)
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
          ...
          const SizedBox(height: 20),
          // Opraveno: Odstraněn .toList() ve spreadu
    ...
    return Padding(
      ...
      child: GlassPanel(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          // TEXT CONTENT (Originál vs Překlad)
          AnimatedSwitcher(
            duration: 300.ms,
            child: Text(
...
// B. JOURNEY SCREEN (RICH GAMIFICATION)
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
        ...
        _buildPathLine(),
        _buildConstellationNode(context, 3, "Strážce", true, state), // Current
  ...
  Widget _buildConstellationNode(BuildContext context, int level, String title, bool unlocked, AppState state) {
    return Row(children: [
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (unlocked && level == 3) Text("Současná úroveň (2450/3000 XP)", style: TextStyle(color: state.moodColor, fontSize: 10))
      ])
    ]);
  }

  Widget _buildPathLine({bool active = false}) {
    return Container(margin: const EdgeInsets.only(left: 19, top: 5, bottom: 5), width: 2, height: 30, color: active ? Colors.white54 : Colors.white10);
  }
}
    ...
    return Center(
      child: SingleChildScrollView(
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            // Opraveno: Opacity je nyní správně definováno
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
        ...
        GlassPanel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text("Pokles Stresu", style: TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 20),
          Row(mainAxisAlignment: MainAxisAlignment.spaceAround, crossAxisAlignment: CrossAxisAlignment.end, children: [0.8, 0.7, 0.6, 0.5, 0.4, 0.3].map((e) => Container(width: 20, height: e * 150, color: Color.lerp(Colors.green, Colors.red, e), margin: const EdgeInsets.symmetric(horizontal: 5))).toList())
        ]))
      ]),
    );
  }
